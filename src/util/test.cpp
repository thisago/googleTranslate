...
int16_t gc_i;
struct js_var newXr(struct js_var key, struct secret_t * secret)
{
    int16_t i;
    i = 0;
    for (;i < js_var_from(JS_VAR_NAN) - 2;(i = i + 3))
    {
        /* Cannot determine variable type from source null*/ ch;
        struct js_var chInt;
        ch = /* Unsupported function call: secret.charAt(i + 2) */;
        chInt = /* Unsupported element access scenario: ch charCodeAt */(0);
        if (js_var_lessthan(chInt, js_var_from_int16_t(str_char_code_at("a", 0))) < 0)
            chInt = js_var_compute(chInt, JS_VAR_MINUS, js_var_from_int16_t(87));
        else
            chInt = str_to_int16_t(ch);
        if (/* unsupported equality expression secret.charAt(i + 1) == "+" */)
            chInt = js_var_compute(key, JS_VAR_USHR, chInt);
        else
            chInt = js_var_compute(key, JS_VAR_SHL, chInt);
        if (/* unsupported equality expression secret.charAt(i) == "+" */)
            key = js_var_plus(key, chInt);
        else
            key = /* unsupported expression key ^ chInt */;
    }
    return key;
